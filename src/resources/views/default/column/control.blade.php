	...
	<div class="text-right pull-right" style="width: 90px;">
		...
		@if ($deletable)
			<form action="{{ $deleteUrl }}" method="POST" style="display:inline-block;">
				...
				<button class="btn btn-danger btn-sm btn-delete" data-toggle="tooltip" title="{{ trans('sleepingowl::core.table.delete') }}">
		...
		@if ($restorable)
			<form action="{{ $restoreUrl }}" method="POST" style="display:inline-block;">
				<input type="hidden" name="_token" value="{{ csrf_token() }}" />
				<button class="btn btn-primary btn-sm" data-toggle="tooltip" title="{{ trans('sleepingowl::core.table.restore') }}">
